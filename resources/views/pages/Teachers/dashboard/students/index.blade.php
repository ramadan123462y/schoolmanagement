...
<h5 style="font-family: 'Cairo', sans-serif;color: red"> {{ trans('pages/attendance.Current Date') }} :
    {{ date('Y-m-d') }}</h5>
<form id="outerForm" method="post" action="{{ url('teacher/attendance/store') }}">
            ...
            @foreach ($students as $student)
                <tr>
                    ...
                    <td>
                        ...
                        @if (isset(
                                $student->attendances()->where('attendence_date', date('Y-m-d'))->first()->id))
                            ...
                            @include('pages.Teachers.dashboard.students.editattend')
                        @else
                            <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
    ...
    <P>
        <button class="btn btn-success" id="submit1" type="submit">{{ trans('pages/attendance.submit') }}</button>
    </P>
</form><br>
...
<script>
   $(document).ready(function() {
      $('#changeActionBtn').click(function(event) {
         event.preventDefault(); // إلغاء سلوك الزر الافتراضي
         // تغيير خاصية "action" للنموذج
         $('#outerForm').attr('action', '{{ url('teacher/attendance/update') }}');
